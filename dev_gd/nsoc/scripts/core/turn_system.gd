...
var _combat: Node                      # 提供 attack_cells / move_card / apply_damage_to_hero
var _spawners: SpawnerSystem
var _card_resolver: Callable

func setup(board: BoardModel, combat: Node, spawners: SpawnerSystem, card_resolver: Callable) -> void:
	_board = board
	_combat = combat
	_spawners = spawners
...
func _run_phase(faction: int) -> void:
	...
	for cell in _board.iter_cells(faction):
		...
		if cell.row == goal_row:
			...
			cell.has_attacked = true
			await _combat.get_tree().create_timer(0.5).timeout
			continue
