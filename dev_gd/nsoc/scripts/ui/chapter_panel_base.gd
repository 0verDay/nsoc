class_name ChapterPanelBase
extends Control

# 章节入口面板基类（重构文档.md §7 阶段 2「结构归一」）。
#
# 背景：changbanpo / weizhenhuaxia / jieting 三份脚本原本互相复制——前两者实测
# 各 490 行、逐行比对只差 14 行（章节名 / 配置路径 / 日志前缀 / 注释）。
# 现在把共用逻辑收敛到本类，子类只提供章节参数：
#     func chapter_name() -> String   章节显示名（标题栏文本）
#     func config_path()  -> String   章节数据 JSON（res://data/chapters/*.json）
#     func log_tag()      -> String   日志前缀（默认取章节名，可覆写成英文标识）
#
# 入场动效：描述面板从左滑入、检阅面板从右滑入、底部进度条从下方滑入；
# 与 CampaignPanel 长按业务的渐隐过渡衔接。
#
# 数据驱动：章节标题 / 描述 / 卡牌列表来自 config_path()，由 campaigns.json 的
# config 字段引用；卡牌 cost / 详情属性反查 res://data/all_cards.json。
#
# 检阅列表交互：长按 → 弹出详情面板（复用 DetailPanelController）；
# 松开 / 拖动 → 收回详情。
...
## 章节数据 JSON 路径（res://data/chapters/*.json），用于标题/描述/卡牌列表。
func config_path() -> String:
	return ""

## 日志前缀，默认取章节名；子类可覆写为英文标识以便检索。
func log_tag() -> String:
	return chapter_name()
...
func _process(_delta: float) -> void:
	...
	match status:
		...
		ResourceLoader.THREAD_LOAD_FAILED, ResourceLoader.THREAD_LOAD_INVALID_RESOURCE:
			push_error(log_tag() + ": failed to load " + NEXT_SCENE_PATH)
			_load_finished = true
...
# 读 config_path()：返回 {description, cards: [{name, count}]}。
# 文件缺失/损坏 → 返回空骨架，UI 仍能空载渲染。
func _load_config() -> Dictionary:
	var empty := {"description": "", "cards": []}
	if not ResourceLoader.exists(config_path()):
		push_warning(log_tag() + ": config missing → " + config_path())
		return empty
...
func _build_review(cfg: Dictionary) -> void:
	...
	for raw in cfg.get("cards", []):
		...
		if card == null:
			push_warning(log_tag() + ": card not in all_cards.json → " + cname)
			continue
		entries.append({
			"name": cname,
