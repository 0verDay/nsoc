...
#
# 数据驱动：
#   章节标题 / 描述 / 卡牌列表来自 res://data/chapters/changbanpo.json，
#   通过 campaigns.json c1.长坂坡.config 字段引用。
...
func _process(_delta: float) -> void:
	...
	match status:
		...
		ResourceLoader.THREAD_LOAD_FAILED, ResourceLoader.THREAD_LOAD_INVALID_RESOURCE:
			push_error("Changbanpo: failed to load " + NEXT_SCENE_PATH)
			_load_finished = true
...
# 读 changbanpo.json：返回 {description, cards: [{name, count}]}。
# 文件缺失/损坏 → 返回空骨架，UI 仍能空载渲染。
func _load_config() -> Dictionary:
	var empty := {"description": "", "cards": []}
	if not ResourceLoader.exists(CONFIG_PATH):
		push_warning("Changbanpo: config missing → " + CONFIG_PATH)
		return empty
...
func _build_review(cfg: Dictionary) -> void:
	...
	for raw in cfg.get("cards", []):
		...
		if card == null:
			push_warning("Changbanpo: card not in all_cards.json → " + cname)
			continue
		entries.append({
			"name": cname,
