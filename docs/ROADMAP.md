...
> 本文件只记录**实时状态**，每次切片完成后更新。最后更新：阶段 2 收尾 —— `main`/`test_main` 合一（基类 `BattleSceneBase`）。

## 一句话状态

**阶段 0 完成；阶段 1/2/3 进行中，整体约完成 75~80%。**
分支 `refactor`（基线 tag `pre-refactor`），60 个提交，全部以"十九套 headless 测试全绿 + 两条黄金路径状态哈希逐位不变"作为行为不变的证据。
