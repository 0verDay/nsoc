...
分支 `refactor`（基线 tag `pre-refactor`），49 个提交，全部以"十六套 headless 测试全绿 + 两条黄金路径状态哈希逐位不变"作为行为不变的证据。
...
| 2 确保无法作弊 | ~97% | **服务器端到端链路已跑通并已被客户端接线**：中继认权威、意图只发权威；权威进程注册/开局/tick/出站；权威规则（落子/整侧行动/终局/法术/技能/装备/费用效果）；客户端 `Net` v2 原语 + `V2BattleClient` + `AuthBoardRenderer` + `Game.enable_v2_authority()` 接线（出牌/结束回合/投降走 `intent/*`，盘面按 `auth/state` 重绘），本地联调 `E2E_RESULT PASS`。**未完成**：**大厅尚未打开这个开关**（没有"为房间起权威进程 + 把牌组/关卡交给权威"的流程），因此人类玩家当前仍走 v1；手牌 UI 仍按本地牌库渲染 |
