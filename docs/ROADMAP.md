...
> 本文件只记录**实时状态**，每次切片完成后更新。最后更新：阶段 3 装备按 auth/state 渲染（`AuthEquipRenderer`）+ 修掉 deck 跨对局残留。
