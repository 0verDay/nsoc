...
分支 `refactor`（基线 tag `pre-refactor`），42 个提交，全部以"十三套 headless 测试全绿 + 两条黄金路径状态哈希逐位不变"作为行为不变的证据。
