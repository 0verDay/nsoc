...
- [x] `is_multi_team_pvp()` 便捷方法（game_context.gd）（统一 1v3/3v3 分支判断）
...
- `BoardRegistry.adjacent_enemy_slots` → 按 team_id 通用

- `BoardOrchestrator` 侧盘位置 → 按 `team_id != local_team` 判上/下

- `_iter_phase_cells_of_slot` → `slot.team_id != ""` 统一 row 0→ROWS-1

- `BoardModel.front_row_of_slot / back_row_of_slot` → `team_id != ""` 统一

- `BoardSlot._on_hero_died` winner 计算 → `pvp_teams.keys()` 动态取，任意队名兼容

- `action/cross_board` 消息队列 → FIFO 通用

- `game/end` 结算逻辑 → `team_of_player(local_player_id) == winning_team`
